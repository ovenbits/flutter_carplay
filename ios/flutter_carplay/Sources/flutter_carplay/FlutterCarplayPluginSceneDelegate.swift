  ...
  CPInterfaceControllerDelegate
{
  ...
  static public func popToRootTemplate(animated: Bool) {
    ...
    self.interfaceController?.popToRootTemplate(animated: animated) { _, error in
      if let error = error {
        NSLog(
          "FlutterCarPlaySceneDelegate - popToRootTemplate error: \(error.localizedDescription)")
      }
    }
  }
  ...
  static public func pushIfNotExist(template: CPTemplate, animated: Bool) -> Bool {
    ...
    if !isAlreadyPushed && !isTopSameInstance {
      interfaceController.pushTemplate(template, animated: animated) { _, error in
        if let error = error {
          NSLog(
            "FlutterCarPlaySceneDelegate - pushTemplate (pushIfNotExist) error: \(error.localizedDescription)"
          )
        }
      }
      return true
